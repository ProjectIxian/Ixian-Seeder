using IxianSeeder.API;
using IxianSeeder.Network;
using IXICore;
using IXICore.Meta;
...
namespace IxianSeeder.Meta
{
    ...
    class Node : IxianNode
    {
        ...
        public void start(bool verboseConsoleOutput)
        {
            ...
            PresenceList.init(IxianHandler.publicIP, Config.serverPort, 'M');
            ...
            if (generatedNewWallet || !File.Exists(Config.walletFile))
            {
                ...
                tiv.start("", false, true);
            }else
            {
                tiv.start("", 0, null, false, true);
            }
